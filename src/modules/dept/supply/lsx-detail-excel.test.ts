...
describe('hai loại file tách riêng (user chốt 06/09/2026)', () => {
  ...
  it('có giá mua gần nhất, tạm tính = còn phải đặt × giá, tổng theo tiền tệ', async () => {
    ...
    expect(d.getCell(17).value).toBe(41_000) // đơn giá gần nhất
    expect(d.getCell(18).value).toBe('VND')
    expect(d.getCell(19).value).toBe(100 * 41_000) // tạm tính = 100 cây × giá
    expect((d.getCell(20).value as Date).toISOString().slice(0, 10)).toBe('2026-07-15')
    expect(d.getCell(21).value).toBe('Kim Phát')
