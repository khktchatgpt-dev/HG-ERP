...
export function TodoScreenV4({
  ...
  const mineCount = useMemo(
    () =>
      meId
        ? groupTodos(pos.filter((p) => p.assigned_to === meId), today).reduce(
            (n, g) => n + g.rows.length,
            0,
          )
        : 0,
        ...
        */
        <Table>
          <THead>
            ...
            <th className="num" style={{ width: 96 }}>Về / tổng</th>
            <th className="num" style={{ width: 124 }}>Tiền hàng</th>
          </THead>
          <tbody>
            {lane.rows.map((po) => (
              <Row key={po.id} onClick={() => undefined}>
                ...
                <Cell num>
                  ...
                  <span className="ml-1 text-[10px] text-[var(--ink-3)]">{po.currency}</span>
                </Cell>
              </Row>
            ))}
          </tbody>
