...
import type { DefectCodeOpt, Stage } from './LogbookScreen'
...
export function FastEntryGrid({
  ...
  useEffect(() => {
    ...
    let alive = true
    api<OutputData>(`/api/dept/production/lsx/${lsxId}/output`)
      .then((d) => {
        if (!alive) return
  ...
  const reasonOpts = defectCodes.filter(
    (c) => c.stage_code === null || c.stage_code === stage,
  )
  ...
  const save = useCallback(async () => {
    ...
    if (bad.length > 0) {
      toast.error('Thiếu nguyên nhân lỗi', `${bad.length} dòng có phế chưa chọn lý do`)
      return
    }
      ...
      for (const g of groups.values()) {
        ...
        try {
          const res = await api<{ warnings: string[] }>(
            `/api/dept/production/lsx/${g.lsxId}/output`,
      ...
      {locked ? (
        ...
        <p className="px-4 py-4 text-xs text-zinc-400">
          {data.components.length === 0
            ? 'Lệnh chưa có bảng chi tiết — Kế hoạch nhập ở Định hình sản xuất trước.'
            : 'Không chi tiết nào đi qua công đoạn này (theo lộ trình đã định hình).'}
        </p>
      ) : (
                  ...
                  >
                    ...
                    <td className="py-1 pr-2">
                      <select
                        ...
                        title={
                          reasonMissing ? 'Phế > 0 phải chọn nguyên nhân' : undefined
                        }
                      >
