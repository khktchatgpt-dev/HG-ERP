...
**Sinh mã chứng từ** (BG-2026-0001, DH-, LSX-, PO-): 
```sql
...
sales_quotes (
  ...
  currency char(3) default 'VND',              -- hedge OI-02, chưa cần tỷ giá
...
alter warehouse_movements
  + warehouse_id uuid FK warehouses default (MAIN),
...
| # | Migration | Mở khoá |
|---|---|---|
| 0011 | `catalogs` (catalog_items + seed, warehouses + movements.warehouse_id, doc_counters + next_doc_code) | mọi thứ sau |
| 0012 | `technical_bom` (alter products + bom_lines) | Kỹ thuật BOM UI |
| 0013 | `sales_quotes_orders` (quotes/lines, orders/lines/changes) | Sales UI + duyệt BG |
| 0014 | `production_orders` (+progress) | phát LSX, xưởng |
| 0015 | `supply` (suppliers, PO/lines, alter movements FK, view po_line_status) | Cung ứng UI + duyệt PO + nối kho |
...
1. `currency char(3) default 'VND'` trên quote/order/PO — hedge OI-02, chưa làm tỷ giá.
