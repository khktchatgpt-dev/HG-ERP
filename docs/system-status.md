...
| **BGĐ** (`/exec`) | ✅ | **CEO/COO theo chuẩn ERP (07/2026 đợt 2)**: `/exec` = **Báo cáo CEO** (cảnh báo đỏ management-by-exception, duyệt one-tap nhúng, đơn trọng điểm + %HT + cờ nghẽn, chart SVG sản lượng 8 tuần + pipeline đơn); `/exec/ops` = **Tháp điều hành COO** (sơ đồ xưởng xanh/vàng/đỏ theo `teamStatusColor`, dòng chảy WIP + badge Nghẽn, chất lượng 7 ngày drill-down 3 tầng tới nguyên nhân phế, cung ứng, resolve sự cố tại chỗ); `/exec/approvals` = **thẻ duyệt mobile-first** (nút to 1 chạm, badge "Giá trị lớn" ≥ 50tr `BIG_APPROVAL_VND`, PoDetail giữ nguyên). Toán thuần ở `src/lib/exec-ops.ts` (18 test); tổng tiền PO chuẩn hoá qua `poLineAmount` (vá bug bỏ qua price_basis unit2). Màn điều hành SX vẫn chỉ admin/manager | Doanh thu/tài chính GĐ2 (MISA); tăng ca đợt sau |
