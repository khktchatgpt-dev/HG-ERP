...
| SAL-05 | Sửa đơn + lịch sử thay đổi | `sales_order_changes` (change jsonb, changed_by); sửa dòng SP/hạn giao sau khi phát LSX → notify Cung ứng + GĐ (event `order.changed_after_lsx`, 07/2026); huỷ đơn khép chuỗi LSX/PO (0036) | DB + app |
| SAL-06 | Phát LSX 1 đơn = 1 LSX, GĐ xác nhận | `production_orders.sales_order_id UNIQUE NOT NULL` + `issued_by/issued_at`; LSX bị từ chối có đường **gửi duyệt lại** (resubmit — giữ BR-01, 07/2026) | **DB (BR-01 ép cứng)** |
...
| SAL-09 | Cảnh báo trễ/thiếu vật tư | `src/lib/late-risk.ts` (pure + test): badge/lọc ⚠ ở tracking + widget sales/planning (07/2026). Notification đẩy theo lịch chờ hạ tầng cron (GĐ2) | ✅ app (hiển thị) |

## Kỹ thuật (FR-ENG) — GĐ1: 01–05

| FR | Yêu cầu | Schema | Mức |
|---|---|---|---|
| ENG-01 | Thư viện SP theo khách | `technical_products.customer_id` (null = mẫu chung); thông tin XK & đặc tính nội thất: HS code, xuất xứ, chất liệu, tải trọng, lắp ráp (KD), bộ gồm (0037) + NW/GW/CBM per thùng trong `packing` (07/2026) | DB + app |
...
| FR | Yêu cầu | Schema | Mức |
|---|---|---|---|
| SUP-01 | Từ LSX + BOM ra nhu cầu VT; cho đặt thủ công khi chưa BOM | `technical_bom_lines` × `sales_order_lines.qty`; từ 07/2026 ƯU TIÊN **bảng chi tiết nhập tay** (`production_order_components` 0038 — kg + số cây, `smartLsxNeeds`), fallback BOM; `po_lines` không ràng BOM (BR-07) | ✅ DB + app |
...
| SUP-04 | Trạng thái từng đơn đặt | enum status đủ chuỗi đặc tả: ordered/confirmed/in_transit/partial/received; **cảnh báo quá hẹn giao** ⚠ (`assessPoLate` — badge/lọc/widget, 07/2026; notification đẩy chờ cron GĐ2) | DB + app |
| SUP-05 | Còn thiếu từng dòng = đặt − nhận | view `supply_po_line_status` tính từ sổ cái, không denorm (BR-08) | DB |
| SUP-06 | Quản lý NCC: thông tin, lịch sử mua, **bảng giá** | `supply_suppliers` + lịch sử = PO theo supplier; bảng giá = `supply_supplier_prices` (0034) + modal Bảng giá + so giá/autofill trong form PO (07/2026) | ✅ DB + app |
| SUP-07 | Hồ sơ mua hàng gắn LSX/đơn đặt | `files.purchase_order_id` (0016) + module files hỗ trợ parent `purchase_order` + khối "Hồ sơ mua hàng" trong chi tiết PO (07/2026) | ✅ DB + app |
...
| WMS-02 | Nhập theo đơn, về từng phần / về đủ | `movements.po_line_id` + view `supply_po_line_status`; service chặn nhập theo PO ngoài RECEIVABLE (chưa duyệt/huỷ/về đủ — 07/2026) | DB + app |
...
| FR | Yêu cầu | Schema | Mức |
|---|---|---|---|
| PROD-01 | Cập nhật giai đoạn LSX | `current_stage` + `production_progress` (log start/done); **xưởng tự thao tác** tại `/production` — quyền `isProductionStaff` theo `departments.workspace_id` (07/2026) | ✅ DB + app |
| PROD-02 | Xác nhận đã nhận VT xuất kho theo LSX | `production_progress.action='received'` (0035) + nút "Đã nhận vật tư" trên chi tiết LSX (07/2026) | ✅ DB + app |
| PROD-03 | Báo hoàn thành → giao hàng | `production_orders.status='completed'` → orders.status; xưởng bấm được từ `/production/lsx/[id]` (07/2026) | ✅ DB + app |
...
| BR-09 xuất LSX phải gắn LSX | **DB check** `warehouse_movements_lsx_link_check` (NOT VALID cho data cũ); service chặn thêm: LSX phải đã duyệt / đang SX mới xuất được (07/2026) | insert `ref_type='lsx'` thiếu FK → lỗi; xuất cho LSX chưa duyệt/huỷ → 400 |
...
| # | Gap | Đề xuất |
|---|---|---|
| G-1 | ~~FR-SUP-06 "bảng giá NCC"~~ | ✅ ĐÓNG 07/2026: `supply_supplier_prices` (0034) + module prices + so giá trong form PO. Flow Excel cũ vẫn chạy song song trong lúc nhập liệu dần |
...
| G-3 | ~~FR-PROD-02: "xưởng xác nhận đã nhận vật tư"~~ | ✅ ĐÓNG 07/2026: `production_progress.action='received'` (0035) + nút trên chi tiết LSX (guard canTrackProgress) |
